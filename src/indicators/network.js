...
var NetworkIndicator = class NetworkIndicator extends CustomButton {
  _init() {
    ...
    /*
        ...
        */
    if (this._network) {
      Main.panel.statusArea.aggregateMenu.menu.box.remove_actor(
        this._network.menu.actor
      );
      this.menu.addMenuItem(this._network.menu);
    }
    if (this._bluetooth) {
      Main.panel.statusArea.aggregateMenu.menu.box.remove_actor(
        this._bluetooth.menu.actor
      );
      this.menu.addMenuItem(this._bluetooth.menu);
    }

    Main.panel.statusArea.aggregateMenu.menu.box.remove_actor(
      this._location.menu.actor
    );
    this.menu.box.add_actor(this._location.menu.actor);

    Main.panel.statusArea.aggregateMenu.menu.box.remove_actor(
      this._rfkill.menu.actor
    );
    this.menu.addMenuItem(this._rfkill.menu);

    this.menu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
    let network = new PopupMenu.PopupMenuItem(_("Network Settings"));
    network.connect("activate", () =>
      this._openApp("gnome-network-panel.desktop")
    );
    ...
    if (this._network) {
      this._network_notify = this._network._primaryIndicator.connect(
        "notify",
        () => this._sync()
      );
    }
    ...
    if (this._network) {
      this.menu.connect("open-state-changed", (menu, isOpen) => {
        if (isOpen) {
          let nmdevices =
            Main.panel.statusArea.aggregateMenu._network._nmDevices;
          ...
          if (
            this._devicewireless &&
            Main.panel.statusArea.aggregateMenu._network._client
  ...
  _onDestroy() {
    ...
    Main.panel.statusArea.aggregateMenu.menu.box.add_actor(
      this._location.menu.actor
    );
...
var WirelessPopupMenuItem = class WirelessPopupMenuItem extends PopupMenu.PopupBaseMenuItem {
  _init(network) {
    ...
    if (
      this._ap._secType != imports.ui.status.network.NMAccessPointSecurity.NONE
    )
  ...
  _getSignalIcon() {
    if (this._ap.mode == imports.ui.status.network.NM80211Mode.ADHOC)
      ...
      return (
        ...
        imports.ui.status.network.signalToIcon(this._ap.strength) +
        "-symbolic"
      );
  }
};
...
var WirelessList = new Lang.Class({
  ...
  _init: function(client, device, settings, parent) {
    ...
    this._wirelessEnabledChangedId = this._client.connect(
      "notify::wireless-enabled",
      Lang.bind(this, this._syncView)
    );

    this._rfkill = Rfkill.getRfkillManager();
    this._airplaneModeChangedId = this._rfkill.connect(
    ...
    this._apAddedId = device.connect(
      "access-point-added",
    ...
    this._activeApChangedId = device.connect(
    ...
    this._scanTimeoutId = Mainloop.timeout_add_seconds(
      15,
      Lang.bind(this, this._onScanTimeout)
    );
    GLib.Source.set_name_by_id(
      this._scanTimeoutId,
      "[gnome-shell] this._onScanTimeout"
    );
    this._onScanTimeout();
  ...
  destroy: function() {
    ...
    if (this._activeApChangedId) {
      GObject.Object.prototype.disconnect.call(
        this._device,
        this._activeApChangedId
      );
  ...
  _activeApChanged: function() {
    if (this._activeNetwork) this._activeNetwork.item.setSelectedIcon(false);

    this._activeNetwork = null;
    if (this._device.active_access_point) {
      let idx = this._findNetwork(this._device.active_access_point);
      if (idx >= 0) this._activeNetwork = this._networks[idx];
    }

    if (this._activeNetwork) this._activeNetwork.item.setSelectedIcon(true);
    this._updateSensitivity();
  },

  _updateSensitivity: function() {
    let connectSensitive =
      this._client.wireless_enabled &&
    ...
    if (network.connections.length > 0) {
      ...
      if (
        accessPoints[0]._secType ==
          imports.ui.status.network.NMAccessPointSecurity.WPA2_ENT ||
        accessPoints[0]._secType ==
          imports.ui.status.network.NMAccessPointSecurity.WPA_ENT
      ) {
        ...
        Util.spawn([
          ...
          this._use_NM ? accessPoints[0].path : accessPoints[0].dbus_path
        ]);
      } else {
        if (this._use_NM) {
          // GNOME >=3.28
          ...
          this._client.add_and_activate_connection(
            connection,
            this._device,
  ...
  _getApSecurityType: function(accessPoint) {
    if (accessPoint._secType) return accessPoint._secType;
    ...
    if (rsn_flags != imports.ui.status.network.NM80211ApSecurityFlags.NONE) {
      ...
      if (
        rsn_flags &
        imports.ui.status.network.NM80211ApSecurityFlags.KEY_MGMT_802_1X
      )
        type = imports.ui.status.network.NMAccessPointSecurity.WPA2_ENT;
      else if (
        rsn_flags &
        imports.ui.status.network.NM80211ApSecurityFlags.KEY_MGMT_PSK
      )
        type = imports.ui.status.network.NMAccessPointSecurity.WPA2_PSK;
    } else if (
      wpa_flags != imports.ui.status.network.NM80211ApSecurityFlags.NONE
    ) {
      if (
        wpa_flags &
        imports.ui.status.network.NM80211ApSecurityFlags.KEY_MGMT_802_1X
      ...
      else type = imports.ui.status.network.NMAccessPointSecurity.NONE;
    }
  ...
  _networkSortFunction: function(one, two) {
    ...
    if (oneHasConnection && !twoHasConnection) return -1;
    else if (!oneHasConnection && twoHasConnection) return 1;

    let oneStrength = one.accessPoints[0].strength;
    let twoStrength = two.accessPoints[0].strength;

    // place stronger connections first
    if (oneStrength != twoStrength) return oneStrength < twoStrength ? 1 : -1;

    let oneHasSecurity =
      one.security != imports.ui.status.network.NMAccessPointSecurity.NONE;
    let twoHasSecurity =
      two.security != imports.ui.status.network.NMAccessPointSecurity.NONE;

    // place secure connections first
    // (we treat WEP/WPA/WPA2 the same as there is no way to
    // take them apart from the UI)
    if (oneHasSecurity && !twoHasSecurity) return -1;
    else if (!oneHasSecurity && twoHasSecurity) return 1;
  ...
  _networkCompare: function(network, accessPoint) {
    ...
    if (
      !this._use_NM &&
      !imports.ui.status.network.ssidCompare(
        network.ssid,
        accessPoint.get_ssid()
      )
    )
      return false;
    if (network.mode != accessPoint.mode) return false;
    if (network.security != this._getApSecurityType(accessPoint)) return false;
  ...
  _findNetwork: function(accessPoint) {
    if (accessPoint.get_ssid() == null) return -1;

    for (let i = 0; i < this._networks.length; i++) {
      if (this._networkCompare(this._networks[i], accessPoint)) return i;
    }
    return -1;
  },
  ...
  _checkConnections: function(network, accessPoint) {
    this._connections.forEach(function(connection) {
      if (
        accessPoint.connection_valid(connection) &&
  ...
  _accessPointAdded: function(device, accessPoint) {
    if (accessPoint.get_ssid() == null) {
      ...
      accessPoint._notifySsidId = accessPoint.connect(
        "notify::ssid",
        Lang.bind(this, this._notifySsidCb)
      );
      return;
    }
    ...
    if (pos != -1) {
      ...
      let newPos = Util.insertSorted(
        this._networks,
        network,
        this._networkSortFunction
      );
  ...
  _resortItems: function() {
    ...
    this._networks.forEach(
      Lang.bind(this, function(network) {
  ...
  _createNetworkItem: function(network) {
    ...
    network.item.connect(
      ...
      Lang.bind(this, function() {
        Util.ensureActorVisibleInScrollView(
          this._scrollView,
          network.item.actor
        );
