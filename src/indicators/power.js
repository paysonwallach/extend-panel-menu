...
var PowerIndicator = class PowerIndicator extends CustomButton {
  _init() {
    ...
    this._settings.connect("activate", () =>
      this._openApp("gnome-power-panel.desktop")
    );
  ...
  _sync() {
    ...
    if (!powertype) {
      ...
      this._brightness.menu.actor.show();
      this._percentageLabel.visible = this._power._desktopSettings.get_boolean(
        "show-battery-percentage"
      );
      this._percentageLabel.clutter_text.set_markup(
        '<span size="smaller">' + this._power._proxy.Percentage + " %</span>"
      );
      ...
      let hideOnFull =
        this._hideOnFull &&
        this._power._proxy.State == UPower.DeviceState.FULLY_CHARGED;
      let hideAtPercent =
        this._hideOnPercent &&
